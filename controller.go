...
func (c *Controller) handleServiceObject(obj interface{}){
	...
	if(sourceNamespace == "staging"){
		...
		if (service.ObjectMeta.Annotations["linkdevtostaging"] != "false"){
			...
			var _, deverr = c.getService(serviceName, "dev")
			if (deverr == nil){
...
func (c *Controller) getService(serviceName string, namespace string) (*corev1.Service, error) {
		...
		var serviceObject *corev1.Service = service.(*corev1.Service);
